...
`v0.4.23`; the checked-in Compose file pins the matching image by immutable digest. There is no
`latest` tag.
...
Codex traffic can cross networks that block direct access.
...
```bash
git clone --branch v0.4.23 --depth 1 \
