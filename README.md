...
```text
docker.io/iraccooni/sowhat-task-worker:0.3.11
docker.io/iraccooni/sowhat-task-worker@sha256:17a461241f8808b668e39901d1ea1b49c0a0336c34422ccd5d0dd353ce64a821
```

Use the digest form. There is deliberately no `latest` tag. Versions `0.2.0` through `0.3.10` are
superseded.
...
```bash
git clone --branch v0.3.11 --depth 1 \
  https://github.com/IRaccoonI/sowhat-task-worker.git
cd sowhat-task-worker
```

Tag `v0.3.11` pins the scripts, AppArmor profiles and Compose file used by worker image `0.3.11`.
...
x-task-worker-image: &task-worker-image docker.io/iraccooni/sowhat-task-worker@sha256:17a461241f8808b668e39901d1ea1b49c0a0336c34422ccd5d0dd353ce64a821
...
- **A dependency CLI such as `prettier` reports `Permission denied` below `/runs`** — use tag
  `v0.3.11` or newer. It pins worker `0.3.11`, which explicitly mounts the disposable task tmpfs with
